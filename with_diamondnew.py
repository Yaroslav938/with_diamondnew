...
with tab_ncbi:
    ...
    with st.expander("🛠 Точные настройки фильтрации (Улучшенный поиск)", expanded=True):
        ...
        exact_title = st.text_input("Обязательное слово в названии (Protein Name)", placeholder="Например: nuclease", help="Искать только те записи, где это слово есть в официальном названии белка.")
    ...
    with col_res1:
        start_index = st.number_input("Начать скачивание с результата №", min_value=1, value=1, help="Например, введите 501, если первые 500 уже скачаны.")
    with col_res2:
        max_results = st.number_input("Сколько результатов скачать", min_value=1, max_value=2000, value=50, help="Максимальное количество последовательностей за один раз.")
    ...
    if st.button("🔍 Искать и скачать последовательности"):
        if not ncbi_email:
            ...
            if exact_title:
                final_query += f' AND "{exact_title}"[Protein Name]'
            if refseq_only:
            ...
            try:
                ...
                with st.spinner("Поиск идентификаторов в NCBI..."):
                    # Добавлен параметр retstart для смещения (API нумерует с 0, поэтому -1)
                    handle = Entrez.esearch(db="protein", term=final_query, retstart=start_index - 1, retmax=max_results)
                ...
                if not id_list:
                    ...
                    st.success(f"Найдено результатов всего: {count}. Скачиваем {len(id_list)} записей (начиная с №{start_index})...")
